...
<template>
  ...
  <div class="top-icon-wrap">
    <template v-if="tempEditing">
      <!-- <span style="color: #999; font-size: 14px; margin-right: 0.5rem">{{ state.stateBollean ? 'Enabled' : 'Disabled' }}</span> <el-switch v-model="state.stateBollean" @change="stateChange" />
      <div class="divide__line">|</div> -->
      <el-button plain type="primary" @click="saveTemp">SaveTemplates</el-button>
...
async function save(hasCover: boolean = false) {
  // Save用户作品的原理和SaveTemplates是相通的，所以这里反过来用Templates示例
  await saveTemp()
  // // 没有任何修改记录则不Save
  ...
  // stat !== 0 ? useNotification('Saved successfully', '可在"My Designs"中查看') : useNotification('Save failed', msg, { type: 'error' })
...
async function saveTemp() {
  ...
  const data = widgetStore.dLayouts
  if (Number(type) == 1) {
    // SaveComponents，GroupElements要保证在最后一位，才能默认选中
    ...
    // TODO：如果SaveComponents不存在Group，则添加Group。该功能待优化
    if (!dWidgets.value.some((x: Record<string, any>) => x.type === 'w-group')) {
      alert('Component must be a Group!')
      return
      // proxy.dWidgets.push(wGroup.setting)
    }
    res = await api.home.saveTemp({ id: tempid, type, title: state.title || 'Untitled Component', data: JSON.stringify(dWidgets.value), width: dPage.value.width, height: dPage.value.height })
  ...
  res.stat != 0 && useNotification('Saved successfully', 'Template content has been updated')
...
async function stateChange(e: string | number | boolean) {
  ...
  stat != 0 && useNotification('Saved successfully', 'Template content has been updated')
}
async function download() {
  if (state.loading === true) {
    useNotification('Exporting', 'Export in progress, please wait')
    return
  }
  state.loading = true
  emit('update:modelValue', true)
  emit('change', { downloadPercent: 1, downloadText: 'Saving data, please wait..' })
  ...
  if (!backEndCapture) {
    ...
    emit('change', { downloadPercent: 100, downloadText: 'Download complete' })
    state.loading = false
  }
  await save(true)
  const { id, tempid } = route.query
  if (!id && !tempid) {
    ...
    useNotification('Save failed', 'This feature may not be supported. Try selecting a template first.', { type: 'error' })
    state.loading = false
    return
  }
  if (backEndCapture) {
    ...
    emit('update:modelValue', true)
    emit('change', { downloadPercent: 1, downloadText: 'Processing data...' })
    let timerCount = 0
    const animation = setInterval(() => {
      if (props.modelValue && timerCount < 75) {
        timerCount += RandomNumber(1, 10)
        emit('change', { downloadPercent: 1 + timerCount, downloadText: 'Compositing image' })
      } else {
    ...
    await _dl.downloadImg(
      ...
      (progress: number, xhr: any) => {
        if (props.modelValue) {
          clearInterval(animation)
          progress >= timerCount && emit('change', { downloadPercent: Number(progress.toFixed(0)), downloadText: 'Generating image' })
        } else {
          xhr.abort()
          state.loading = false
        }
    ...
    emit('change', { downloadPercent: 100, downloadText: 'Download complete', downloadMsg: '' })
    state.loading = false
  }
}
...
async function load(cb: () => void) {
  ...
  if (route.name !== 'Draw') {
    await useFontStore.init() // 初始化加载Font
  }
  const apiName = tempId && !id ? 'getTempDetail' : 'getWorks'
  if (w_h && !id && !tempId) {
    // 用于初始化画布Size，Create空作品
