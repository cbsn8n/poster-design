...
async function prepare() {
  ...
  if (Number(type) == 1) {
    // SaveComponents，GroupElements要保证在最后一位
    ...
    // TIP：上传Components必须将所有LayersGroupGroup
    if (!dWidgets.value.some((x: Record<string, any>) => x.type === 'w-group')) {
      alert('Please group all layers before uploading')
      return
    }
  }
  ...
  if (page.backgroundImage) {
    emit('change', { downloadPercent: 1, downloadText: 'Preparing to upload', downloadMsg: 'Please wait..' })
...
async function uploadImgs() {
  if (queue.length > 0) {
    ...
    emit('change', { downloadPercent, downloadText: 'Uploading resources', downloadMsg: `Completed: ${lens - queue.length} / ${lens}` })
    item.imgUrl = url
    uploadImgs()
  } else {
    uploadTemplate()
